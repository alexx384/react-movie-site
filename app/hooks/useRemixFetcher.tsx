...
export interface JSONSuccessResponse<T> extends JSONResponse<T> {
  data: T;
}

export interface JSONErrorResponse<T = any> extends JSONResponse<T> {
...
export const useRemixFetcher = ({ onError, onSuccess }: Options = {}) => {
  ...
  React.useEffect(() => {
    const hasFetcherFinishedSubmission =
      fetcher.type === 'done' && fetcher.state === 'idle';
